...
class CustomDataset(Dataset):
    def __init__(
        ...
        self.Y = torch.tensor(
            timeseries_data[model.target_features].values, 
            dtype=torch.float32
            )
        ...
        if model.input_static_features is not None:
            ...
            repeats = torch.tensor(timeseries_data["id"].
                                   astype(str).
            ...
            static_data_order = torch.repeat_interleave(
                static_data_order, repeats, dim=0
                )
            
            self.X = torch.cat([self.X, static_data_order], dim=1)
    ...
    def __len__(self):
        num_batches = int(
            round(
                (len(self.X) - self.warmup_length) / 
                (self.sequence_length - self.warmup_length) + 
                0.49
                )
            )
        return num_batches
